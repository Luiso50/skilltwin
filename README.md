...
**Auditoría actual:** se ha corregido el aislamiento de sesiones de conversación por usuario y se han añadido pruebas de autorización/propiedad. La siguiente prioridad es revisar la migración legacy JSON→SQLite y ampliar la cobertura HTTP de seguridad.
...
- CSRF de un solo uso para operaciones sensibles, incluido el chat y la limpieza de memoria.
- Rate limiting configurable.
- Validación y sanitización de entradas.
- Autorización diferenciada entre admin y customer.
- Sesiones de conversación vinculadas a `user_id` + `clone_id` para evitar acceso cruzado entre clientes.
...
El aislamiento de sesiones de conversación ya está implementado: una sesión se registra con su propietario y clon, y las rutas de historial, chat y limpieza verifican esa relación. Además, `chat-clon` y `clon-limpiar-memoria` exigen CSRF válido.
...
1. Completar tests HTTP de autorización de extremo a extremo entre usuarios.
