...
class ServerConfigTests(unittest.TestCase):
    ...
    def test_chat_session_access_rejects_other_user(self):
        import server
        from dep_operaciones import database

        database.DB_PATH = os.path.join(self.tmpdir.name, 'chat_sessions.db')
        database.init_database()
        database.registrar_chat_session('session-owner', 'clone-001', 100)

        handler = object.__new__(server.CerebroHandler)
        with patch.object(handler, 'send_error_response') as send_error:
            result = handler.require_chat_session_access(
                {'role': 'customer', 'user_id': 200},
                'clone-001',
                'session-owner'
            )
        ...
        send_error.assert_called_once_with(
            'No tienes acceso a esta sesión de conversación.', 403
        )

    def test_chat_session_access_accepts_owner(self):
        import server
        from dep_operaciones import database
        ...
        handler = object.__new__(server.CerebroHandler)
        with patch.object(server.database, 'tocar_chat_session') as touch:
            result = handler.require_chat_session_access(
                {'role': 'customer', 'user_id': 100},
                'clone-001',
                'session-owner'
            )

        self.assertEqual(result, 'session-owner')
        touch.assert_called_once_with('session-owner')

    def test_chat_session_access_creates_session_for_new_customer(self):
        import server
        from dep_operaciones import database
        ...
        self.assertEqual(
            database.obtener_chat_session(result)['user_id'],
            100
        )
