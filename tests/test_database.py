...
class DatabaseTests(unittest.TestCase):
    ...
    def test_chat_session_ownership_crud(self):
        session = database.registrar_chat_session("sess-001", "clone-001", 101)
        self.assertEqual(session["user_id"], 101)
        self.assertEqual(session["clone_id"], "clone-001")

        loaded = database.obtener_chat_session("sess-001")
        self.assertIsNotNone(loaded)
        self.assertTrue(database.validar_propietario_chat_session("sess-001", "clone-001", 101))
        self.assertFalse(database.validar_propietario_chat_session("sess-001", "clone-001", 202))
        self.assertFalse(database.validar_propietario_chat_session("sess-001", "clone-002", 101))

        database.tocar_chat_session("sess-001")
        self.assertTrue(database.eliminar_chat_session("sess-001", 101))
        self.assertIsNone(database.obtener_chat_session("sess-001"))
        self.assertFalse(database.eliminar_chat_session("sess-001", 101))

    def test_chat_session_no_cambia_de_propietario(self):
        first = database.registrar_chat_session("sess-002", "clone-001", 101)
        second = database.registrar_chat_session("sess-002", "clone-001", 202)
        self.assertEqual(first["user_id"], 101)
        self.assertEqual(second["user_id"], 101)
